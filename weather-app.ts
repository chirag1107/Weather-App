...
const mockModeToggle = document.getElementById("mockModeToggle") as HTMLInputElement;
...
function getWeatherIconSvg(mainCondition: string): string {
...
function updateBackgroundTheme(condition: string): void {
...
async function getWeather(city: string): Promise<void> {
  const weatherDiv: HTMLDivElement = document.getElementById("weather") as HTMLDivElement;
  ...
  try {
    ...
    if (isMockMode) {
      ...
      const url: string = `https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(city)}&appid=${savedApiKey}&units=metric`;
